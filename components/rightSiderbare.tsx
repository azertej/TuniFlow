  ...
  return (
    <section className=" sticky right-0 top-0 h-screen w-[350px] background-light900_dark200 light-border custom-scrollbar overflow-y-auto border-l shadow-light-300 dark:shadow-none max-xl:hidden  pt-32 p-6 ">
      <div className="flex flex-col">
        <div>
          <span className="text-2xl font-bold dark:text-slate-200">Top Questions</span>
          <div className="flex flex-col gap-y-3 mt-3">
            {questions.map((question) => {
              return (
                ...
                >
                  <span className="dark:text-white">{question.firstQuestion} </span>
                  <Image
        ...
        <div className="mt-4">
          <span className="text-2xl font-bold dark:text-slate-200">Popular Tags</span>
          <div className="flex flex-col gap-y-3 mt-3">
            {tags.map((tag)=>{
              return(
               <Tags _id={tag._id} tagName={tag.tagName} totalTgas={tag.totalTags} isShowed />
              )
            })}
          </div>
        </div>
